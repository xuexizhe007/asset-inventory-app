...
object PdfGenerator {
    ...
    fun generateLabelsPdf(context: Context, taskName: String?, assets: List<Asset>) {
        ...
        assets.forEachIndexed { index, asset ->
            ...
            // 需求2：二维码更大（在不改变布局的前提下，只减少右侧区域的留白）
            // 原来：(qrAreaWidth - padding * 2)
            // 现在： (qrAreaWidth - padding)  => 明显更大，但仍严格不越界
            ...
            fun drawSingleLine(label: String, value: String) {
                // 这里保持原来的“整行 drawText”写法不动，保证版式稳定
            ...
            // 需求3：第二行改为 “资产名称 + 使用部门”同一行（替换原来的两段 drawWrapped）
            ...
            // 存放地点：允许换行（保持原逻辑）
            drawWrapped("存放地点", asset.location.orEmp
